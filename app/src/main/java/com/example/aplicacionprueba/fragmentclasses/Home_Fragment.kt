...
import com.example.aplicacionprueba.adapters.HomeExhibitorAdapter
...
class Home_Fragment : Fragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        //Ocultamos la progress bar y mostramos la interfaz
    ...
    fun showUpComing(view: View): Boolean {
        RVUpcoming = view.findViewById(R.id.recyclerView) //Inicializamos el RecyclerView

        //Generamos la llamada a la API, el metodo siempre sera un GET, asignamos la key de la API y el lenguaje en el que queremos que nos devuelvan los datos.
        val client = ServiceGenerator.createService(MoviesClient::class.java)
        ...
        call.enqueue(object : Callback<Movies> {
            override fun onResponse(call: Call<Movies>, response: Response<Movies>) {
                val repos = response.body()
                try {
                    //Dentro de la llamada, si es correcta, asignamos el adaptador al RecyclerView
                    RVUpcoming!!.layoutManager = LinearLayoutManager(activity, LinearLayoutManager.HORIZONTAL, false)
                    RVUpcoming!!.adapter =
                        HomeExhibitorAdapter(context!!, repos!!.results, 4)
                } catch (ise: IllegalStateException) {
                }
            }
            override fun onFailure(call: Call<Movies>, t: Throwable) {
            ...
            override fun onResponse(call: Call<Movies>, response: Response<Movies>) {
                ...
                try {
                    ...
                    RVMostPopular!!.adapter =
                        HomeExhibitorAdapter(context!!, repos!!.results, 5)
